...
from school_bot.bot.services.group_service import get_group_by_chat_id, add_group, update_group
...
async def on_bot_added_to_group(event: ChatMemberUpdated, session: AsyncSession) -> None:
    ...
    if group:
        ...
        group_title = chat.title or "Noma'lum"
...
async def group_join_select_school(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if group:
        await update_group(session, group, name=chat.title, school_id=school.id)
    else:
        await add_group(session, name=chat.title, chat_id=chat_id, school_id=school.id)
