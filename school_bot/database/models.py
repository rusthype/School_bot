...
class User(Base):
    ...
    profile: Mapped["Profile | None"] = relationship(
        ...
        uselist=False,
        foreign_keys="Profile.bot_user_id",
    )
...
class PollVote(Base):
    ...
    task_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("bot_tasks.id", ondelete="CASCADE"),
        nullable=True,
        index=True,
    )
    user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("bot_users.id", ondelete="CASCADE"), index=True)
...
class Book(Base):
    ...
    category_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("bot_book_categories.id", ondelete="CASCADE"),
        index=True,
    )
...
class SupportTicket(Base):
    ...
    user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("bot_users.id", ondelete="CASCADE"), nullable=False, index=True)
...
class Profile(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    # Django column name is bot_user_id (OneToOneField named bot_user → db column bot_user_id)
    bot_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("bot_users.id", ondelete="CASCADE"), unique=True, index=True)
    first_name: Mapped[str] = mapped_column(Text, nullable=False)
    last_name: Mapped[str | None] = mapped_column(Text, nullable=True)
    phone: Mapped[str] = mapped_column(Text, nullable=False)
    profile_type: Mapped[str | None] = mapped_column(Text, nullable=True, index=True)
    assigned_groups: Mapped[list] = mapped_column(
        JSON,
        default=list,
    )
...
class TeacherAttendance(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), default=func.now(), nullable=False, index=True)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
        onupdate=func.now(),
...
class Group(Base):
    ...
    school_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("bot_schools.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), default=func.now(), nullable=False)
